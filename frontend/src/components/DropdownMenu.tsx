...
export default function DropdownMenu({ label, children }: { label: string, children: JSX.Element | JSX.Element[] }) {
  return (
    <div className='dropdown relative mx-1'>
      ...
      >
        <div className='flex items-center max-w-[50px] xs:max-w-full'><span className='text-ellipsis overflow-hidden'>{label}</span></div>
        <div className='flex items-center'><ArrowDown className='ml-2 w-2'/></div>
      </button>
